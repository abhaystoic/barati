...
class My_Orders(Dashboard, View):
   try:
         ...
         if orders:
            for order in orders:
               if order.product_type == 'card':
                  ...
                  self.order_details = m.Orders.objects.get(ref_id=order.ref_id)
                  self.delivery_status = m.Delivery_Status.objects.get(ref_id=order.ref_id)
      ...
      def get(self, request, **kwargs):
         ...
         print self.order_details.vendor_acknowledgement
